...
import com.example.tiennguyen.essay.service.PlayerService;

public class SongPlayerActivity extends Activity implements OnClickListener {
    ...
    public void onCreate(Bundle savedInstanceState) {
        ...
        initViews();
        playerService = new Intent(SongPlayerActivity.this, PlayerService.class);
        if (linkFileEdt.getText().toString().compareTo("") != 0) {
            playerService.putExtra("songDestination_URL", linkFileEdt.getText().toString());
        } else {
            playerService.putExtra("songDestination_URL", MUSIC_LINK);
