...
import com.example.tiennguyen.essay.SongPlayerActivity;
import com.example.tiennguyen.essay.util.Utilities;
...
public class PlayerService extends Service implements OnCompletionListener,
    ...
    public int onStartCommand(Intent intent, int flags, int startId) {
        initUI();
        String songDestination_URL = intent.getExtras().getString("songDestination_URL");
        Log.d("SongDestination_URL", "SongDestination_URL = " + songDestination_URL);
    ...
    private void initUI() {
        songCurrentDurationLabel = new WeakReference<TextView>(SongPlayerActivity.songCurrentDurationLabel);
        songTotalDurationLabel = new WeakReference<TextView>(SongPlayerActivity.songTotalDurationLabel);

        btnPlay = new WeakReference<ImageView>(SongPlayerActivity.btnPlay);
        btnForward = new WeakReference<ImageView>(SongPlayerActivity.btnForward);
        btnBackward = new WeakReference<ImageView>(SongPlayerActivity.btnBackward);

        btnPlay.get().setOnClickListener(this);
        btnForward.get().setOnClickListener(this);
        btnBackward.get().setOnClickListener(this);

        songProgressBar = new WeakReference<SeekBar>(SongPlayerActivity.songProgressBar);
        songProgressBar.get().setOnSeekBarChangeListener(this);
    }

    //Gửi tin nhắn tới Activity rằng bài hát đã được lưu vào vùng nhớ đệm
    ...
    //Gửi tin nhắn tới Activity rằng bài hát đã sẵn sàn để play
            ...
            case R.id.btn_play:
                if (mp.isPlaying()) {
                    if (mp != null) {
                        ...
                        btnPlay.get().setImageResource(R.drawable.ic_star_black_24dp);
                    ...
                    if (mp != null) {
                        ...
                        btnPlay.get().setImageResource(R.drawable.ic_audiotrack_black_24dp);
    ...
    public void playSong(String songPath) {
        ...
        if (!mp.isPlaying()) {
            try {
                ...
                btnPlay.get().setImageResource(R.drawable.ic_close_black_24dp);
